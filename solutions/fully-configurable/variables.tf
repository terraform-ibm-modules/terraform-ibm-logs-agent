...
variable "logs_agent_iam_custom_endpoint" {
  type        = string
  default     = null
  description = "Custom IAM endpoint hostname (without https://) for regions requiring region-specific IAM endpoints (e.g., `iam.eu-fr2.cloud.ibm.com`). Only used when `logs_agent_iam_environment` is set to `Custom`."
